...
main_loop(void)
{
	...
	/* Set the time when ultari started */
	if (!started_time) {
	...
	/* Reset the firewall (cleans it, in case we are restarting after ultari crash) */
	iptables_fw_destroy();
	...
	result = pthread_create(&tid, NULL, thread_ultarictl, (void *)(config->ultarictl_sock));
	if (result != 0) {
		debug(LOG_ERR, "FATAL: Failed to create thread_ultarictl - exiting");
		termination_handler(1);
	}

	result = pthread_join(tid, NULL);
	if (result) {
		debug(LOG_INFO, "Failed to wait for ultari thread.");
	}
	MHD_stop_daemon(webserver);
	termination_handler(result);
}

/** Main entry point for ultari.
