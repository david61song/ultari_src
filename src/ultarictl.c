 ...
 * @brief Print usage
 *
 * Prints usage, called when ultarictl is run with -h or with an unknown option
 */
static void
usage(void)
{
	printf(
		"Usage: ultarictl [options] command [arguments]\n"
		"\n"
		"options:\n"
...
connect_to_server(const char sock_name[])
{
	...
	if (connect(sock, (struct sockaddr *)&sa_un, strlen(sa_un.sun_path) + sizeof(sa_un.sun_family))) {
		fprintf(stderr, "ultarictl: ultari probably not started (Error: %s)\n", strerror(errno));
		return -1;
	}
...
send_request(int sock, const char request[])
{
	...
	while (len != strlen(request)) {
		...
		if (written == -1) {
			fprintf(stderr, "Write to ultari failed: %s\n", strerror(errno));
			exit(1);
		}
		len += written;
	}
...
/* Perform a ultarictl action, with server response Yes or No.
 ...
 */
static int
ultarictl_do(const char *socket, const struct argument *arg, const char *param)
{
	int sock;
	char buffer[4096];
	char request[128];
	...
	if (arg->ifyes && arg->ifno) {
		...
		if (rlen < 0) {
			fprintf(stderr, "ultarictl: Error reading socket: %s\n", strerror(errno));
			ret = 3;
			...
			fprintf(stderr, "ultarictl: Error: ultari sent an abnormal reply.\n");
			ret = 2;
		}
	} else {
...
main(int argc, char **argv)
{
	...
	return ultarictl_do(socket, arg, argv[i+1]);
}
