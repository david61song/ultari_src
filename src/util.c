...
static void
ultarictl_json_client(FILE *fp, const t_client *client, time_t now)
{
...
static void
ultarictl_json_one(FILE *fp, const char *arg)
{
	t_client *client;
	time_t now;
	...
	if (client) {
		fprintf(fp, "{\n");
		ultarictl_json_client(fp, client, now);
		fprintf(fp, "}\n");
	} else {
		fprintf(fp, "{}\n");
	}
...
void
ultarictl_json(FILE *fp, const char *arg)
{
	if (arg && strlen(arg)) {
		ultarictl_json_one(fp, arg);
	} else {
		ultarictl_json_all(fp);
	}
}
